...
![[Picture/39163b24f9e3e906434b24a3ef859b28_MD5.png]]

![[Picture/643385b706641c34b8ddb25b93d2a03f_MD5.png]]
...
**NODE\_LOCAL**：节点本地化，代码和数据在同一个节点中；比如说，数据作为一个HDFS block块，就在节点上，而task在节点上某个executor中运行；或者是，数据和task在一个节点上的不同executor中；数据需要在进程间进行传输  
**NO\_PREF**：对于task来说，数据从哪里获取都一样，没有好坏之分  
**RACK\_LOCAL**：机架本地化，数据和task在一个机架的两个节点上；数据需要通过网络在节点之间进行传输  
...
如果大多都是PROCESS\_LOCAL，那就不用调节了  
如果是发现，好多的级别都是NODE\_LOCAL、ANY，那么最好就去调节一下数据本地化的等待时长  
...
![[Picture/ac4c15b8de78f6996426470f8b1d83b0_MD5.png]]
